...
-- A projection with 3 columns (reversed alphabetical order)

SELECT tm_name, tm_city, tm_goals FROM team ORDER BY tm_name DESC;

-- Restriction queries with multiple clauses demonstrating: teams with players from specific country, teams that begin with letters Ba, players that are not from Germany, born from MAR - NOV
...
-- Join queries with data from: 2 tables, 3 tables, more tables
...
-- Show the total number of teams per city.

SELECT tm_city, COUNT(tm_city) "Teams per city" FROM team GROUP BY tm_city;

-- Which team has the worst goal difference (goals scored – goals received)?
...
-- Show the player with the highest contract per position.
...
-- Assume a username for the footballers is made up of the first 2 letters of their country of origin, added to the first 4 letters of their first name added to the first 4 letters of their surname. 
...
-- Show the total of the footballers’ contracts of the league (sum of all contracts of all players for all the teams).

SELECT SUM(ft_contract) AS total_contracts FROM footballers;

-- Which sponsor spends the most money for advertising (sponsoring fees)?
...
-- Which type of business spends the most money for advertising (sponsoring fees)?
...
-- Show the footballers with the highest average contracts per sponsor. Assume that footballers have the same sponsors as their team.
...
-- Which team gets the most from the ‘ELECTRONICS’ sponsors?
